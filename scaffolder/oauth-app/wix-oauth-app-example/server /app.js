...
const { getSiteInfo } = require('./wixApi');
...
*/
const { WIX_REDIRECT_URL, APP_ID, APP_SECRET } = process.env;
...
app.get('/api/dashboard', async (req, res) => {
  ...
  if (verifyInstance(instance, APP_SECRET)) {
    ...
    const siteInfo = await getSiteInfo(wixRefreshToken)

    res.json({ dataJson, siteInfo });
  } else {
